...
function onDrop(x, y, draginfo)
	return CombatManager.onDrop("ct", getDatabaseNode().getNodeName(), draginfo)
end
...
function update()
	...
	if(participatedNode and participatedNode.getValue()==1)then
		Debug.chat("participant has participated")
		createParticipationResultBox()
	else
		Debug.chat("participant has not participated")
...
function destroyParticipationResultBox()
	if(participation_result_box)then
		delete(participation_result_box)
	end
	if(spacer2)then
		delete(spacer2)
	end
